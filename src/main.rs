...
fn Home() -> Element {
    ...
    // let mut validator_memo = use_memo(move || validator_select);
    let mut url = use_signal(|| "http://127.0.0.1:8899".to_string());
    rsx! {
        ...
        div {
            {
                match validator_select.read().as_str()   {
                    "local" => {
...
// fn SelectValidator(validator_select: Signal<String>) -> Element {
//     rsx! {
//         label { r#for: "validator", "Choose a validator:" }
//         select {
//             name: "validator",
